export async function GET(request: Request, { params }: { params: Promise<{ tokenId: string }> }) {
  const { tokenId } = await params  // <- Add 'await' here

  try {
    // Generate mood based on tokenId and current time for variety
    const seed = parseInt(tokenId) + Math.floor(Date.now() / 30000) // Changes every 30 seconds
    const moodIndex = seed % 3
